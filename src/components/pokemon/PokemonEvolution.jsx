...
export const PokemonEvolution = ({ pokemon, evolutionChain, onPokemonSelect }) => {
  ...
  const processEvolutionChain = async (chain, evolutions) => {
    try {
      ...
      const pokemonData = await fetch(`https://pokeapi.co/api/v2/pokemon/${speciesId}`).then(res => res.json());
  ...
  const handleEvolutionSelect = async (evolution) => {
    try {
      const selectedPokemon = await fetch(`https://pokeapi.co/api/v2/pokemon/${evolution.id}`).then(res => res.json());
  ...
  const getEvolutionRequirement = (evolution) => {
          ...
          <div key={evolution.id} className="animate-slide-in-right" style={{ animationDelay: `${index * 0.1}s` }}>
            ...
            <Button
              ...
              className={`w-full p-3 hardware-button ${
                evolution.id === pokemon.id ? 'bg-pokedex-green/20 border-pokedex-green' : ''
              }`}
            >
              <div className="flex items-center space-x-3">
                ...
                <div className="flex-1 text-left">
                  <div className="font-digital font-bold text-sm">
                    {formatPokemonName(evolution.species)}
                  </div>
